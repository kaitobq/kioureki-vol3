...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ organizations }) => {
            ...
            {isOpen && (
              <ul className="absolute bg-red-600 shadow-lg rounded w-auto flex flex-col items-center left-1/2 transform -translate-x-1/2">
                {organizations.map((org) => (
                  <li
                    key={org.id}
                    className={`m-1 px-4 py-2 cursor-pointer rounded-lg ${currentOrganization === org ? "bg-gray-800  hover:bg-gray-600" : "bg-blue-800 hover:bg-blue-600"}`}
                    onClick={() => {
